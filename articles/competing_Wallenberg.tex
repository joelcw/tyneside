...
\citet{baileywallenbergwurff2012} suggest the Proto-Gmc dual pronoun which is the ancestor of \textsl{whether} was reanalyzed as a polarity \textsl{wh}-word in clauses containing a disjunction. In sentences such as the Old English examples below (\
...
\subsection{h-Dropping}

Baranowsky \& Turton (\textsl{to appear}) \noteme{get ref} present a quantitative study of syllable-initial [h]-deletion in Manchester, and show that while the phenomonen is variable, it is remarkably stable across an apparent time distribution.
As the phenomenon is characterized by the presence or absence of initial [h] in the relevant set of words, it is plausibly a case of categorical variation in the phonological domain which could potentially specialize in the way we have described above.
