...
Suppose she assigns/associates any token of either A or B, whichever she happens to hear, to context $C$ (i.e. beginning to specialize it for that context) with probability $q$.
If the child hears a token of B, she will assign B to a specific context or function with probability $q$.
At the same time, since the Principle of Contrast is about \textsl{contrasting} variants, if the child assigns B to context $C$, then she simultaneously assigns A to context $not C$.
We will refer to this process as \textbf{assignment}, and specialization of variants for different contexts is the result of iterative \textbf{assignment} during acquisition.
However, if the child hears a token of A, she will fail to hear it $p$ amount of the time, and so will only successfully specialize A for a context with probability $(1-p)q$.
Clearly, as the selectional advantage increases, the rate of specialization will slow, as the probability for specializing any given token will decrease.
