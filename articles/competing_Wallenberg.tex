...
Free-generate-and-filter grammars make it difficult to localize optionality in the way necessary formulate a competing grammars analysis because it isn't possible to parameterize the grammar in a simple way.
...
\end{tabular}
\end{exe}
In classic Optimality Theory, these constraints lead to a set of 7 different crucial rankings
...
A quantitative study of embedded polar questions in English and Icelandic shows that the same doublet can resolve by \textbf{specialization} in one language or speech community, but by \textbf{replacement} in another.
...
\citet{baileywallenbergwurff2012}, following \citet{van2009}, suggest the Proto-Gmc dual \textsl{wh}-pronoun which is the ancestor of \textsl{whether} was reanalyzed as a polarity operator in clauses containing a disjunction.
In sentences such as the Old English examples in (\ref{oe1})-(\ref{oe2}), the embedded question is ambiguous between a polar or object-question reading, making the ancestor of \textsl{whether} ambiguous between its original function as a dual \textsl{wh}-pronoun and its modern incarnation as a polarity operator:

\begin{exe}
    \ex paste in from thing
\end{exe}

\noindent The reanalysis is illustrated schematically in (\ref{reanalysis}) below.
...
Once this reanalysis occurred, a doublet or competing grammars situation began in historical English and historical Icelandic (and possibly other Germanic varieties, though we do not currently have detailed evidence to this effect).
The outcomes predicted by our theory of change are specialization and replacement, and they both occur: the former in English and the latter in Icelandic.
