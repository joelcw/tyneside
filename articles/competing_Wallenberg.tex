...
(Note that every doublet has this property, with respect to some context, at the point in history just after it is innovated, since no specialization has occurred yet.)
...
The Principle of Contrast comes into the acquisition process when the child decides to change the selection dynamics, by actively privileging one of the variants in a particular context, which we might call $C_i$ (where $C_i$ is only one out of many possible contexts where the variation shows itself, $C_1$...$C_n$).
Again, encountering a piece of A or B data in context $C$, the child can decide that whichever variant they heard is actually specifically associated with $C$ (or, they can simply ignore the context and just count the occurrence of A or B as usual).
