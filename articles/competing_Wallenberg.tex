...
%CORN AND GRAIN, REPLACEMENT IN UK SPECIALIZATION IN US
...
\author{Josef Fruehwald \& Joel C. Wallenberg}
...
%melted/molten example going back to OE, now fully specialized
...
\ex {\it if}/{\it whether} in embedded questions\\
	\Treek[3]{3}{
		&\K{[$_{CP}$ whether [$_{C'}$  if  [ \ldots ] ] ]}\AR{dl}_{p}\AR{dr}^{(1-p)}\\
...
\subsection{The Problem of Stable Variation}
...
Beginning with \citet{clark1987, 1990}, we see a plausible explanation for the existence of the Blocking Effect that Kroch used to explain the instability of morphological doublets.
In studies of word learning, Clark (and much subsequent research; see \citealt{bionetal2013} for a nice review of the literature) identified a strategy children use during first language acquisition to map new linguistic forms onto meanings or functions, the ``Principle of Contrast''.
...
Of course, it is possible for children to acquire synonyms, but experimental studies, such as \citet{markmanwachtel1988} and others reviewed above, provide evidence that this is never the first hypothesis when a child is acquiring two phonologically distinct word forms; rather, when presented with novel words, children try to associate them with meanings that are currently unmapped to words they already know.%todo: fix this section, especially in the light of Joel's recent talks
...
Just as the Principle of Contrast pushes competing lexical items to specialize in meaning, we posit that it pushes competing functional heads and grammatical rules to similarly specialize.
...
Suffice it to say, they may be purely linguistic (e.g. constituent heaviness, polarity of embedded questions), purely social (e.g. language choice in a bilingual setting \notejoe{ok a ref for this}), or a mixture of the two.
...
\subsection{A Model for Specialization in Acquisition}
\label{catspec}

\subsection{Expanding Variational Acquisition to Specialization}
\label{varExpand}

%todo: fix intro to this. Two main points of having this here: if stuff generally replaces, what are the conditions that have to hold in order to get specialization? Specialization is replacement, just context specificfirst, domain of specialization. Next, probability uncoupling. Next, stuff. Next community converges. to create there's a race, and we want to say what specialization is
...
\subsubsection{The Active Specialization Hypothesis}
...
Of course, in the case above specialization only results if the rate of assignment is rapid enough (i.e. the learner-created bias in $C_i$ is strong enough) for A to replace B in $C_i$ before the global selective pressure in favor of B causes B to replace A everywhere, including $C_i$.
...
%revising here
\subsubsection{The ``acceleration of specialization''}
...
\subsubsection{The Passive Specialization Hypothesis}
...
%todo: make sure we make the point in conclusions that ANY OVERLAP IMPLIES COMPETITION, so it's not that the variants have to "mean the exact same thing"
