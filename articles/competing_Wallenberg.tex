...
We will now present two specific hypotheses about how specialization might proceed from this point.
The first is what we will call the \textbf{active specialization} hypothesis, in which the Principle of Contrast represents an active effort by the child acquiring the variation to make the variants diverge in meaning or context.
Under this hypothesis, the child decides to change the selection dynamics, by actively privileging one of the variants in a particular context, which we might refer to as $C_i$ (where $C_i$ is only one out of many possible contexts where the variation shows itself, $C_1$...$C_n$).
...
\notejoel{Should I say something here about passive vs active assignment?}
