...
\subsubsection{More on optionality}
...
Let's revisit the English topicalization construction, as shown in the final sentences of (\ref{princetop1}) and \ref{princetop2}) below.
...
This type of specialization simply cannot go to completion, in the sense above.


In order to test these ideas, we built a computational model of the acquisition and generational transmission of sociolinguistic variation which encodes only the simplest and most empirically motivated assumptions about how acquisition proceeds.
...
The model allows the hypothetical learner to specialize the variants, by allowing them to take different values along the continuous dimension, but it does not enforce specialization. 
...
\begin{figure}
 \centering
 \subfloat[Replacement]{\def\svgscale{0.6}\import{figures/}{replacement.pdf_tex}}
  \qquad
 \subfloat[Specialization]{\def\svgscale{0.6}\import{figures/}{specialization.pdf_tex}}
  \qquad
 \subfloat[Partial Specialization]{\def\svgscale{0.6}\import{figures/}{partial_specialization.pdf_tex}}
 \caption{The possible outcomes of competition. Outcome (c), partial specialization, represents apparent stable variation.}
 \label{competition_figure}
\end{figure}
